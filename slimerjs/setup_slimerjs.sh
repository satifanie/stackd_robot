#!/bin/bash
export SLIMERJS_VERSION_F=0.10.1
export CASPERJS_VERSION_F=1.1.3
export FIREFOX_VERSION_F=49.0.2
curl -sL https://deb.nodesource.com/setup_6.x | sudo -E bash -

apt-get update && \
  apt-get upgrade -y && \
  apt-get install -y unzip git wget xvfb libxrender-dev libasound2 libdbus-glib-1-2 libgtk-3-0 bzip2 python && \
  apt-get install -y nodejs python python-pip python-requests && \
